"""Train SAC + decoupled trigger-gated latent correction on pick-place-v3.

Key difference from train_triggered.py:
  The correction module is trained on a FROZEN base latent so the base actor
  is independently optimised.  This prevents the co-adaptation failure seen
  in the original triggered design (alpha diverging to ~40, near-zero success).

See src/agents/sac_decoupled.py for the full design rationale.

Usage:
    python scripts/train_decoupled.py
    python scripts/train_decoupled.py --seed 1 --run-name sac_pp_decoupled_s1 --wandb
"""
import sys
import os
...
def main():
    ...
    parser.add_argument("--config",   type=str, default="configs/pick_place_decoupled.yaml")
